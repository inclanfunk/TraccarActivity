...
public class Protocol {
	 ...
	 */
	public static String createLocationMessage(boolean extended, Location l,
			boolean inDanger, double battery) {
		...
		if (extended) {
			if (inDanger) {
				//Fire warning, Y
				f.format("%1$tY%1$tm%1$td%1$tH%1$tM%1$tS.%1$tL,A,Y,", calendar);
			}else{
				//Location is N
				f.format("%1$tY%1$tm%1$td%1$tH%1$tM%1$tS.%1$tL,A,N,", calendar);
			}
			...
			if (inDanger) {
				//Fire warning, A
				f.format("%1$tH%1$tM%1$tS.%1$tL,A,Y,", calendar);
			}else{
				//Location is V
				f.format("%1$tH%1$tM%1$tS.%1$tL,A,N,", calendar);
			}
			...
			f.format("%02d%07.4f,%c,", (int) Math.abs(lat),
					Math.abs(lat) % 1 * 60, lat < 0 ? 'S' : 'N');
			f.format("%03d%07.4f,%c,", (int) Math.abs(lon),
					Math.abs(lon) % 1 * 60, lon < 0 ? 'W' : 'E');
